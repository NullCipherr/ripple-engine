...
describe('FluidEngine backend alternativo', () => {
  it('mantém backend webgpu-experimental como ativo (sem fallback)', () => {
    const gl = createMockWebGL2Context();
    const canvas = createCanvasMock(gl);
    const engine = new FluidEngine(canvas, createConfig({ renderBackend: 'webgpu-experimental' }), {
      enableDomInput: false,
    });
    engine.initialize();

    const status = engine.getRenderBackendStatus();
    expect(status.requested).toBe('webgpu-experimental');
    expect(status.active).toBe('webgpu-experimental');
    expect(status.isFallback).toBe(false);
    engine.dispose();
  });

  it('troca backend em runtime via updateConfig', () => {
    const gl = createMockWebGL2Context();
    const canvas = createCanvasMock(gl);
    const config = createConfig({ renderBackend: 'classic' });
    const engine = new FluidEngine(canvas, config, { enableDomInput: false });
    engine.initialize();
    expect(engine.getRenderBackendStatus().requested).toBe('classic');

    engine.updateConfig({ ...config, renderBackend: 'webgpu-experimental' });
    expect(engine.getRenderBackendStatus().requested).toBe('webgpu-experimental');
    engine.dispose();
  });
});
