...
describe('FluidEngine input desacoplado de DOM', () => {
  it('permite processar input programático com DOM input desativado', () => {
    const gl = createMockWebGL2Context();
    const canvas = createCanvasMock(gl);
    const engine = new FluidEngine(canvas, createConfig(), { enableDomInput: false });

    engine.initialize();
    engine.reset();
    engine.processInput({ normX: 0.5, normY: 0.5, normDx: 0.01, normDy: -0.01, injectDensity: true });
  ...
  it('move obstáculos por índice via API de engine', () => {
    const gl = createMockWebGL2Context();
    const canvas = createCanvasMock(gl);
    const engine = new FluidEngine(canvas, createConfig(), { enableDomInput: false });
    engine.initialize();
    engine.updateObstacles([{ id: 'o1', type: 'circle', x: 0.2, y: 0.3, radius: 0.06 }]);

    const moved = engine.moveObstacleByIndex(0, 0.8, 0.9);
    expect(moved[0].x).toBe(0.8);
    expect(moved[0].y).toBe(0.9);
    engine.dispose();
  });
});
